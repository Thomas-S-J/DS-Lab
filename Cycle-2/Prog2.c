...
int hashcode(char* word){
	...
	int hash_value = (ascii_ch1 * ascii_ch2)%TABLE_SIZE_l;
...
int search_linear(char *word){
	...
	int flag = 0;
	int start_index = hashcode(word);
	int count = 1;
	int current_index = start_index;

	do{
		if(strcmp(hash_table_l[current_index], word) == 0){
			flag = 1;
			printf("Found the word in the linear-probing hash table.\n");
			return count;
		}
		count++;
	...
	if(flag == 0){
		...
		printf("The word was not found in the linear-probing hash table.\n");
	}
...
int search_quadratic(char *word){
	...
	do{
		...
		if(strcmp(hash_table_q[current_index], word) == 0){
			flag = 1;
			printf("The word was found in the quadratic-probing hash table.\n");
			return count;
		}
		count++;
	...
	if(flag == 0){
		printf("The word was not found in the hash table where we inserted using quadratic probing.\n");
	}
