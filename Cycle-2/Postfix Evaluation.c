...
char perform_operation(int operand1, int operand2, char operator){
	...
	/*
	 * TODO:Very inefficient ? Even I felt so...but I wasn't getting
...
int evaluate_postfix_expression(char *postfix_expression){
	...
	for(int i = 0; postfix_expression[i]; i++){
		 ...
		 * is either an operator or an operand. Hence the postfix expression
		 * is a character array (as it has to store symbols like '+', '*' etc),
		 * but the result of the evaluation is an integer...so we store the
		 * stack (which stores only operands and the result of evaluation at
		 * different stages) as an integer array.
		 */
		...
		if(isOperator(current_symbol)){
			 ...
			 */
			TOP_STACK++;
			stack[TOP_STACK] = current_symbol - '0';
		}
	}
	/*
